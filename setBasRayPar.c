...
/*   Updated: 2021/04/26 20:00:03 by rvena            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void	findWall(t_ray_data *raycasting, char **map)
{
...
static void	setBasRayPar3(t_ray_data *ray,
				t_plr_data *player, char **map, int x)
{
	findWall(ray, map);
	if (ray->side == 0)
...
void	setBasRayPar2(t_ray_data *ray, t_plr_data *player, char **map, int x)
{
	if (ray->rayDirX < 0)
	{
		ray->stepX = -1;
...
void	setBasRayPar(t_ray_data *ray, t_all_data *ever, int x, char **map)
{
	...
	ray->cameraX = 2 * x / (double)t_settings->sW - 1;
